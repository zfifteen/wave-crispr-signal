...
class PhaseAwareSpectralAnalyzer:
    ...
    def calculate_phase_difference_features(self, sequence: str, 
                                         mutation_pos: int = None,
                                         mutation_base: str = None) -> Dict[str, float]:
        """
        ...
        for metric in ['entropy', 'flatness', 'f1_magnitude']:
            ...
            phase_diff_features[f'delta_phase_{metric}_change'] = delta_mutated - delta_original
        
        return phase_diff_features
    
    def _compute_spectral_features(self, sequence: str, phase: int = 0) -> Dict[str, float]:
        """
    ...
    def _calculate_geodesic_resolution(self, n: int, phase: int, k: float = 0.3) -> float:
        """
...
class GoldenProximityCalculator:
    ...
    def calculate_golden_proximity(self, z_values: List[float], 
                                 trim_outliers: bool = True) -> Dict[str, float]:
        """
        ...
        if trim_outliers and len(z_values) > 2:
            ...
            trimmed_values = z_array[(z_array >= lower_bound) & (z_array <= upper_bound)]
...
class CurvatureDisruptionAnalyzer:
    ...
    def calculate_curvature_disruption(self, sequence: str, mutation_pos: int,
                                     mutation_base: str, window_size: int = 25) -> Dict[str, float]:
        """
        ...
        original_features = self._calculate_curvature_features(original_window, mutation_pos - start_pos)
        mutated_features = self._calculate_curvature_features(mutated_window, mutation_pos - start_pos)
        
        # Calculate disruption metrics
        disruption_metrics = {}
        for feature_name in original_features:
            delta_curv = mutated_features[feature_name] - original_features[feature_name]
    ...
    def _calculate_curvature_features(self, sequence: str, focal_pos: int) -> Dict[str, float]:
        """
...
class InvariantFeatureSet:
    ...
    def calculate_complete_feature_set(self, sequence: str, mutation_pos: int = None,
                                     mutation_base: str = None) -> Dict[str, Union[float, int]]:
        """
