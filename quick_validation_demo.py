...
class QuickValidator:
    ...
    def run_validation(self) -> Dict:
        ...
        # Analyze results
        bio_significant = sum(
            1 for r, p in bio_correlations.values() if abs(r) >= 0.3 and p < 0.05
        )
        arb_significant = sum(
            1 for r, p in arb_correlations.values() if abs(r) >= 0.3 and p < 0.05
        )
    ...
    def print_results(self, results: Dict):
        ...
        print(
            f"{'Feature':<20} {'Correlation':<12} {'P-value':<10} {'Significant':<12}"
        )
        print("-" * 60)
        ...
        print("\n🎯 SUMMARY")
        print(
            f"Biological encoder significant correlations: {results['biological_significant']}"
        )
        print(
            f"Arbitrary encoder significant correlations: {results['arbitrary_significant']}"
        )

        if results["hypothesis_supported"]:
            print(
                "✅ HYPOTHESIS SUPPORTED: Biological encoding shows superior performance"
            )
        else:
            print(
                "❌ HYPOTHESIS NOT SUPPORTED: No clear advantage for biological encoding"
            )
    ...
    def create_visualization(self, results: Dict):
        ...
        significant_counts = [
            results["biological_significant"],
            results["arbitrary_significant"],
        ]
        ...
        plt.tight_layout()
        plt.savefig(
            "/home/runner/work/wave-crispr-signal/wave-crispr-signal/quick_validation_results.png",
            dpi=150,
            bbox_inches="tight",
        )
