...
def evaluate(df: pd.DataFrame, r: float = 20.0, folds: int = 5, alpha: float = 1.0) -> Dict[str, Dict[str, List[float]]]:
...
def format_stats(x: List[float]) -> str:
...
def main():
    ...
    out = evaluate(df, r=args.r, folds=args.folds, alpha=args.alpha)
        #
        return f"{np.mean(x):.4f} ± {np.std(x):.4f}"
    print("Baseline    R2:", format_stats(out["baseline"]["r2"]))
    print("+Breathing  R2:", format_stats(out["+breathing"]["r2"]))
    print("ΔR2:", f"{(np.mean(out['+breathing']['r2']) - np.mean(out['baseline']['r2'])):.4f}")
    print("Baseline   rho:", format_stats(out["baseline"]["rho"]))
    print("+Breathing rho:", format_stats(out["+breathing"]["rho"]))
    if len(out["baseline"]["auprc"]):
        print("Baseline  AUPRC:", format_stats(out["baseline"]["auprc"]))
        print("+Breath.  AUPRC:", format_stats(out["+breathing"]["auprc"]))
