...
def compute_fft_spectrum(
    waveform: np.ndarray, remove_dc: bool = True
...
def compute_fundamental_frequency(spectrum_mag: np.ndarray, freqs: np.ndarray) -> float:
    """
...
def count_sidelobes(
    spectrum_mag: np.ndarray, freqs: np.ndarray, threshold_percentile: float = 90.0
) -> int:
    """
...
def analyze_disruption(
    ...
    # Encode sequences
    ref_waveform = phase_weighted_encoding(
        reference_sequence, is_rna=is_rna, phi=phi, k=k
    )
    mut_waveform = phase_weighted_encoding(mutant_sequence, is_rna=is_rna, phi=phi, k=k)

    # Compute features
    features = compute_spectral_features(
        mut_waveform, reference_waveform=ref_waveform, sequence=mutant_sequence
    )
    ...
    logger.info(
        f"Disruption analysis: Δf₁={features.get('delta_f1', 0):.4f}, "
...
def batch_analyze_disruption(
    ...
    for i, (mut_seq, ref_seq) in enumerate(zip(mutant_sequences, reference_sequences)):
        try:
            features = analyze_disruption(mut_seq, ref_seq, is_rna=is_rna, phi=phi, k=k)
