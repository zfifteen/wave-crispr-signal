...
def bootstrap_confidence_interval(
    ...
    """
    if n_bootstrap < 1000:
        logger.warning(f"Bootstrap samples ({n_bootstrap}) below recommended minimum of 1,000")
...
def score_with_confidence(
    ...
    logger.info(
        f"Composite score: {mean_score:.4f} "
...
def auto_optimize_k(
    sequence: str,
    k_candidates: Optional[List[float]] = None,
    is_rna: bool = False
) -> float:
    """
...
def compare_to_baseline(
    ...
    logger.info(
        f"Lift: {lift:.4f} (95% CI: [{ci_lower:.4f}, {ci_upper:.4f}]), "
        f"p={p_value:.4f}"
    )
