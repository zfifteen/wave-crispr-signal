...
# Golden ratio for phase weighting
...
def encode_sequence(
    ...
    logger.debug(f"Encoded {'RNA' if is_rna else 'DNA'} sequence of length {len(sequence)}")
    
    return waveform


def geometric_phase_weight(
    n: int,
    phi: float = 21.0,
    k: float = 0.3
) -> float:
...
def phase_weighted_encoding(
    ...
    phase_weights = np.array([
        geometric_phase_weight(n, phi=phi, k=k)
