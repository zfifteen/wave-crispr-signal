...
from experiments.spectral_disruption_profiler.analysis import analyze_disruption, batch_analyze_disruption
...
def load_sequences_from_csv(filepath: str, ref_col: str = 'reference', mut_col: str = 'mutant') -> List[Dict[str, str]]:
...
def cmd_score(args):
    ...
    logger.info(f"Reference: {reference_seq[:50]}..." if len(reference_seq) > 50 else f"Reference: {reference_seq}")
    logger.info(f"Mutant: {mutant_seq[:50]}..." if len(mutant_seq) > 50 else f"Mutant: {mutant_seq}")
...
def cmd_batch(args):
    ...
    with open(output_path, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=[
            'id', 'reference', 'mutant',
            'delta_f1', 'delta_entropy', 'delta_sidelobes',
            'gc_content', 'composite_score', 'off_target_flag'
        ])
        writer.writeheader()
...
def main():
    ...
    score_parser.add_argument('--reference', required=True, help='Reference sequence or FASTA file')
    score_parser.add_argument('--mutant', required=True, help='Mutant sequence or FASTA file')
    score_parser.add_argument('--output', default='results.json', help='Output JSON file')
    score_parser.add_argument('--rna', action='store_true', help='Use RNA encoding (U instead of T)')
    score_parser.add_argument('--phi', type=float, default=21.0, help='Geometric period (default: 21)')
    score_parser.add_argument('--k', type=float, default=0.3, help='Curvature parameter (default: 0.3)')
    score_parser.add_argument('--auto-k', action='store_true', help='Auto-optimize k parameter')
    
    # Batch command
    batch_parser = subparsers.add_parser('batch', help='Batch process multiple sequences')
    batch_parser.add_argument('--input', required=True, help='Input CSV file with reference and mutant columns')
    ...
    batch_parser.add_argument('--phi', type=float, default=21.0, help='Geometric period')
    batch_parser.add_argument('--k', type=float, default=0.3, help='Curvature parameter')
    batch_parser.add_argument('--bootstrap', type=int, default=1000, help='Bootstrap resamples (default: 1000)')
    batch_parser.add_argument('--permutations', type=int, default=1000, help='Permutations for GC resonance (default: 1000)')
    batch_parser.add_argument('--seed', type=int, default=42, help='Random seed (default: 42)')
