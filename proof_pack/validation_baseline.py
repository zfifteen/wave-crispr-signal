...
class BaselineFeatureExtractor:
    ...
    def extract_sequence_features(self, sequences: List[str]) -> np.ndarray:
        ...
        for seq in sequences:
            ...
            for base in ["A", "T", "G", "C"]:
                ...
                seq_features[f"{base}_freq"] = (
                    base_counts.get(base, 0) / total_bases if total_bases > 0 else 0
                )

            # GC content
            gc_count = base_counts.get("G", 0) + base_counts.get("C", 0)
            seq_features["gc_content"] = (
                gc_count / total_bases if total_bases > 0 else 0
            )
            ...
            for dinuc in dinucs:
                ...
                seq_features[f"dinuc_{dinuc}"] = (
                    count / (total_bases - 1) if total_bases > 1 else 0
                )
        ...
        for seq in numeric_sequences:
            ...
            if len(seq_array) == 0:
                ...
                seq_features = [
                    ...
                    (
                        np.mean(np.diff(seq_array)) if len(seq_array) > 1 else 0
                    ),  # Mean difference
                    np.std(np.diff(seq_array)) if len(seq_array) > 1 else 0,  # Diff std
                    len(np.unique(seq_array)),  # Unique values
                    np.sum(seq_array > np.mean(seq_array))
                    / len(seq_array),  # Above mean ratio
                ]
    ...
    def extract_histogram_features(
        self, numeric_sequences: List[List[int]], bins: int = 20
    ) -> np.ndarray:
    ...
    def extract_fourier_features(
        self, numeric_sequences: List[List[int]], n_components: int = 5
    ) -> np.ndarray:
...
class PoissonnNullModel:
    ...
    def generate_null_sequences(
        self, n_samples: int, mean_length: int = 100
    ) -> List[str]:
    ...
    def generate_null_spikes(
        self, n_samples: int, mean_rate: float = 10.0
    ) -> List[Dict]:
