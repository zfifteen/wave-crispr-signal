...
def generate_nav18_panel_data(n_samples=500):
    ...
    for i in range(n_samples):
        ...
        nav17_affinity = nav18_affinity * np.random.lognormal(2.0, 0.5)  # Less selective
        nav16_affinity = nav18_affinity * np.random.lognormal(2.5, 0.6)  # Even less selective
...
def generate_bcl11a_edits_data(n_samples=300):
    ...
    for i in range(n_samples):
        ...
        gc_content = (grna_sequence.count('G') + grna_sequence.count('C')) / len(grna_sequence)
        ...
        hbf_induction = base_hbf + (30 * efficiency_factor * gc_factor * off_target_penalty)
