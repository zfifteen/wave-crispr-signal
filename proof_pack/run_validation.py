...
from pain_management_application import PainManagementAnalyzer, theta_prime, compute_density_boost
...
class ZFrameworkValidator:
    ...
    def load_synthetic_data(self) -> Dict[str, pd.DataFrame]:
        ...
        for file in data_files:
            ...
            if filepath.exists():
                ...
                self.logger.info(f"Loaded {len(data[file.replace('.csv', '')])} samples from {file}")
            else:
    ...
    def extract_z_framework_features(self, sequences: List[str], 
                                   numeric_sequences: List[List[int]]) -> Dict[str, np.ndarray]:
        ...
        for seq_num in numeric_sequences:
            ...
            for k in k_values:
                if len(seq_num) > 0:
                    theta_vals = [float(theta_prime(n, k)) for n in seq_num[:50]]  # Limit length
                    seq_features.extend([
                        np.mean(theta_vals),
                        np.std(theta_vals),
                        np.max(theta_vals),
        ...
        for i, seq in enumerate(sequences):
            try:
                ...
                result = self.pain_analyzer.implement_z5d_predictor(seq, target_n=10**4)  # Smaller target for speed
                z5d_feature = [
    ...
    def run_classification_validation(self, X_baseline: np.ndarray, X_z: np.ndarray, 
                                    y: np.ndarray, task_name: str) -> Dict[str, Any]:
        ...
        for model_name, model in models.items():
            ...
            _, p_value = stats.mannwhitneyu(y_pred_base, y_pred_z, alternative='two-sided')
    ...
    def run_regression_validation(self, X_baseline: np.ndarray, X_z: np.ndarray, 
                                y: np.ndarray, task_name: str) -> Dict[str, Any]:
    ...
    def validate_density_boost_claims(self, numeric_sequences: List[List[int]]) -> Dict[str, Any]:
    ...
    def generate_validation_report(self):
        ...
        if 'neural_spikes' in data:
            ...
            numeric_sequences = self.parse_numeric_sequences(neural_data['sequence_numeric'].tolist())
            
            # Extract features
            baseline_features = self.baseline_extractor.extract_all_features(sequences, numeric_sequences)
        ...
        if 'neural_spikes' in self.results:
            ...
            if 'classification' in results:
                ...
                for model, metrics in results['classification'].items():
                    ...
                    print(f"  Significant:      {'✓' if metrics['significant'] else '✗'}")
            ...
            if 'density_boost' in results:
                ...
                print(f"  95% CI:           [{density['ci_95_lower']:.1f}, {density['ci_95_upper']:.1f}]")
                ...
                print(f"  Significant:      {'✓' if density['significant_boost'] else '✗'}")
    ...
    def save_results(self):
        ...
        if 'neural_spikes' in self.results:
            ...
            if 'classification' in self.results['neural_spikes']:
                for model, metrics in self.results['neural_spikes']['classification'].items():
                    summary_data.append({
                        'task': 'classification',
                        'model': model,
                        'metric': 'AUC',
            ...
            if 'regression' in self.results['neural_spikes']:
                for model, metrics in self.results['neural_spikes']['regression'].items():
                    summary_data.append({
                        'task': 'regression',
                        'model': model,
                        'metric': 'RMSE',
