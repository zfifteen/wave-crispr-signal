...
def analyze_file(filepath):
    ...
    try:
        ...
        # Print results
        print(
            f"   📈 Mean fold-change: {mean_fold:.1f}x (95% CI: {mean_ci_low:.1f}-{mean_ci_high:.1f})"
        )
        print(
            f"   📊 Median fold-change: {median_fold:.1f}x (95% CI: {med_ci_low:.1f}-{med_ci_high:.1f})"
        )
        print(
            f"   📏 Range: {np.min(fold_changes):.1f}x to {np.max(fold_changes):.1f}x"
        )
        print(
            f"   🚀 >1000x success: {success_1000}/{len(fold_changes)} ({success_1000/len(fold_changes):.1%})"
        )
        print(
            f"   💯 >100x success: {success_100}/{len(fold_changes)} ({success_100/len(fold_changes):.1%})"
        )
        print(
            f"   🔟 >10x success: {success_10}/{len(fold_changes)} ({success_10/len(fold_changes):.1%})"
        )
...
def create_summary_plot(results_list, output_dir=None):
    ...
    try:
        ...
        if output_dir:
            plt.savefig(
                Path(output_dir) / "validation_summary.png",
                dpi=150,
                bbox_inches="tight",
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate Z Framework density boost claims"
    )
    parser.add_argument(
    ...
    if results_list:
        ...
        print(
            f"🚀 Overall >1000x success: {total_1000x}/{total_samples} ({overall_1000x_rate:.1%})"
        )
