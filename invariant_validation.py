...
class InvariantValidator:
    ...
    def calculate_phase_stability_index(self, sequences: List[str], 
                                       num_bootstrap: int = 100) -> Dict[str, float]:
        """
        ...
        for _ in range(num_bootstrap):
            ...
            for seq in resampled_sequences:
                try:
                    features = self.invariant_features.calculate_complete_feature_set(seq)
        ...
        results = {
            'phase_stability_entropy': calc_stability_index(delta_phase_entropy_samples),
            'phase_stability_flatness': calc_stability_index(delta_phase_flatness_samples),
    ...
    def stratify_mutation_analysis(self, sequences: List[str], 
        ...
        results = {
            ...
            'other_transitions': {
                'delta_phase_entropy': calc_ci(other_transitions, 'delta_phase_entropy'),
                'delta_phase_flatness': calc_ci(other_transitions, 'delta_phase_flatness'),
        ...
        if gc_transitions and other_transitions:
            ...
            if len(gc_entropy) > 1 and len(other_entropy) > 1:
                ...
                t_stat, p_value = stats.ttest_ind(gc_entropy, other_entropy, equal_var=False)
    ...
    def evaluate_guide_performance_lift(self, sequences: List[str], 
                                      use_invariants: bool = True,
                                      baseline_scoring: bool = False) -> Dict[str, float]:
        """
        ...
        relative_improvement = (mean_inv - mean_base) / mean_base if mean_base > 0 else 0
        ...
        results = {
            ...
            'effect_size': (mean_inv - mean_base) / np.sqrt((np.var(invariant_scores) + np.var(baseline_scores)) / 2)
        }
        
        return results
    
    def comprehensive_validation_report(self, sequences: List[str],
                                      num_bootstrap: int = 50) -> Dict[str, any]:
        """
    ...
    def _generate_summary(self, report: Dict) -> Dict[str, any]:
        ...
        elif phase_quality in ["High", "Moderate"] or summary.get('improvement_percentage', 0) > 5:
