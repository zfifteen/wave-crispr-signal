...
class TestPredictorComparison(unittest.TestCase):
    ...
    def test_predictor_differences(self):
        ...
        for n in test_values:
            ...
            self.assertLess(
                abs(z5d_result - li_result) / max(z5d_result, li_result), 0.5
            )
    ...
    def test_z5d_advantage_large_n(self):
        ...
        for n, known_prime in test_cases:
            ...
            if n >= 10**7:
                ...
                self.assertLess(
                    z5d_error, li_error, f"Z5D should outperform LI for n={n:,}"
                )
            else:
                # For smaller n, LI may edge out (pre-asymptotic regime)
                # Both should be reasonable though
                self.assertLess(
                    z5d_error, 0.01, f"Z5D should be reasonable for n={n:,}"
                )
...
class TestZ5DPerformanceExperiment(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.experiment = Z5DPerformanceExperiment(
            min_n=1000, max_n=10000, num_samples=5
        )
    ...
    def test_predictor_testing(self):
        ...
        results = self.experiment.test_predictor(
            self.experiment.z5d_predictor, n_values
        )
    ...
    def test_hypothesis_confirmation_large_n(self):
        ...
        large_n_experiment = Z5DPerformanceExperiment(
            min_n=10**7, max_n=10**8, num_samples=3
        )
        ...
        self.assertLess(
            result.z5d_mean_error,
            result.li_mean_error,
...
def run_validation_tests():
    ...
    for n in test_values:
        ...
        print(
            f"n={n:5d}: Z5D={z5d_result:7.1f} (err={z5d_error:.4f}), "
    ...
    print(
        f"Statistical significance: p = {result.statistical_significance['p_value']:.4f}"
    )
