...
mp.dps = 50
LEGACY_ARTIFACTS_DIR = Path("legacy/artifacts")
...
class FalsificationExperiment:
    ...
    def generate_test_sequences(
        self, n_sequences: int = 100, length: int = 1000
    ) -> List[str]:
        """
    ...
    def test_k_parameter_claim(self, sequences: List[str]) -> Dict:
        ...
        results = {
            ...
            "enhancement_data": df,
            "claimed_k_enhancement": (
                claimed_k_performance["mean_enhancement"].values[0]
                if len(claimed_k_performance) > 0
                else 0
            ),
            "baseline_k_enhancement": (
                baseline_k_performance["mean_enhancement"].mean()
                if len(baseline_k_performance) > 0
                else 0
            ),
        }
        ...
        results.update(
            {
                ...
                "enhancement_ratio_claimed_vs_optimal": (
                    results["claimed_k_enhancement"] / optimal_enhancement
        ...
        print("✅ k* parameter test complete:")
        print(
            f"   Claimed k* = {self.k_claimed} gives enhancement = {results['claimed_k_enhancement']:.4f}"
        )
        print(
            f"   Actual optimal k = {optimal_k:.4f} gives enhancement = {optimal_enhancement:.4f}"
        )
    ...
    def test_density_boost_claim(self, sequences: List[str]) -> Dict:
        ...
        min(10000, len(sequences[0]) * 10)  # Scale down from 10^6
        ...
        for seq in sequences[:20]:  # Test on multiple sequences
            try:
                ...
                enhanced_density = self._calculate_geodesic_density(
                    seq, k=self.k_claimed
                )

                # Calculate boost percentage
                if baseline_density > 0:
                    boost_pct = (
                        (enhanced_density - baseline_density) / baseline_density * 100
                    )
        ...
        if density_boosts:
            ...
            results = {
                ...
                "boost_claim_falsified": p_value < self.alpha
                and mean_boost < 50.0,  # Far below 210%
                "max_observed_boost": max(density_boosts),
                "min_observed_boost": min(density_boosts),
            }

            print("✅ Density boost test complete:")
            print("   Claimed boost: 210%")
    ...
    def test_biological_relevance(self, sequences: List[str]) -> Dict:
        ...
        if real_results and random_results:
            ...
            print("✅ Biological relevance test complete:")
...
def run_complete_falsification_study():
    ...
    report_path = LEGACY_ARTIFACTS_DIR / "FALSIFICATION_REPORT.md"
    results_path = LEGACY_ARTIFACTS_DIR / "falsification_results.json"

    with open(report_path, "w") as f:
        f.write(report)

    experiment.save_results(str(results_path))
