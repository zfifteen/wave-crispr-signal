...
class ZFrameworkCalculator:
    ...
    def __init__(self, precision_dps: int = 50):
        ...
        logger.info(
            f"Initialized Z Framework Calculator with {precision_dps} decimal precision"
        )
    ...
    def calculate_delta_n(self, sequence_values: List[mp.mpf], position: int) -> mp.mpf:
        ...
        if position >= len(sequence_values):
            raise IndexError(
                f"Position {position} exceeds sequence length {len(sequence_values)}"
            )
    ...
    def calculate_z_values(self, sequence: str) -> Dict[str, mp.mpf]:
        ...
        logger.info(
            f"Z mean: {z_mean}, convergence to φ-1: {phi_conjugate_convergence}"
        )
        logger.info(
            f"Z variance: {z_variance}, convergence to target: {variance_convergence}"
        )

        return results

    def calculate_density_enhancement(
        self, sequence: str, window_size: int = 10
    ) -> Dict[str, mp.mpf]:
        """
        ...
        for i in range(n - window_size + 1):
            ...
            z_results = self.calculate_z_values(
                "".join(["ATCG"[int(v) - 1] for v in window_values])
            )
        ...
        results = {
            ...
            "density_variance": sum((d - mean_density) ** 2 for d in density_values)
            / mp.mpf(len(density_values)),
            "enhancement_variance": sum(
                (e - mean_enhancement) ** 2 for e in enhancement_values
            )
            / mp.mpf(len(enhancement_values)),
        }

        return results

    def perform_falsification_test(
        self,
        sequence: str,
        ...
        for i in range(num_perturbations):
            ...
            try:
                ...
                phi_convergence = abs(
                    perturbed_results["z_mean"] - self.phi_conjugate
                ) < mp.mpf("0.05")
                var_convergence = abs(
                    perturbed_results["z_variance"] - TARGET_VARIANCE
                ) < mp.mpf("0.05")
        ...
        logger.info(
            f"Falsification test completed: {len(perturbed_means)}/{num_perturbations} successful"
        )
...
def format_mpmath_for_json(value):
    """Convert mpmath values and numpy types to JSON-serializable types."""
