...
class BiologicalAnchoredEncoder:
    ...
    def _compute_encodings(self):
        ...
        for nucleotide, props in NUCLEOTIDE_PROPERTIES.items():
            ...
            real_part = amplitude * np.cos(phase) + real_part/10  # scale real component
...
class ExperimentalValidator:
    ...
    def run_experiment(self, sequences: List[str], 
                      efficiencies: List[float]) -> Dict[str, any]:
    ...
    def _extract_features_batch(self, sequences: List[str], encoder) -> pd.DataFrame:
        ...
        for seq in sequences:
            try:
                ...
                features_list.append({key: np.nan for key in 
                                    ['spectral_entropy', 'peak_magnitude', 'mean_magnitude',
                                     'spectral_variance', 'spectral_std', 'peak_frequency',
                                     'bandwidth', 'spectral_centroid']})
        
        return pd.DataFrame(features_list)
    
    def _test_correlations(self, features_df: pd.DataFrame, 
                          efficiencies: List[float]) -> Dict[str, Tuple[float, float]]:
    ...
    def _bootstrap_validation(self, sequences: List[str], efficiencies: List[float], 
        ...
        for i in range(self.num_bootstrap):
            ...
            try:
                ...
                boot_correlations = self._test_correlations(boot_features, boot_efficiencies)
    ...
    def print_results(self):
        ...
        print(f"\n📊 DATASET SUMMARY")
        print(f"Total sequences analyzed: {self.results['experiment_params']['num_sequences']}")
        print(f"Bootstrap iterations: {self.results['experiment_params']['num_bootstrap']}")
        print(f"Significance threshold: {self.results['experiment_params']['significance_level']}")
        
        print(f"\n🔬 BIOLOGICALLY ANCHORED ENCODER RESULTS")
        print(f"{'Feature':<20} {'Correlation (r)':<15} {'P-value':<12} {'Significant':<12}")
        print("-" * 60)
        
        for feature, (r, p) in bio_results['correlations'].items():
            significant = "✓" if (abs(r) >= 0.5 and p < self.significance_level) else "✗"
        ...
        print(f"\n📈 STATISTICAL COMPARISON")
        print(f"Biologically anchored significant features: {comparison['summary']['bio_significant_features']}")
        print(f"Arbitrary mapping significant features: {comparison['summary']['arb_significant_features']}")
        print(f"Biological encoder advantage: {'✓' if comparison['summary']['bio_advantage'] else '✗'}")
        
        print(f"\n🎯 HYPOTHESIS TEST CONCLUSION")
        if comparison['summary']['bio_advantage']:
            print("✅ HYPOTHESIS SUPPORTED: Biologically anchored encodings show superior")
            print("   predictive performance compared to arbitrary mappings.")
            print(f"   ({comparison['summary']['bio_significant_features']} vs {comparison['summary']['arb_significant_features']} significant correlations)")
        else:
            print("❌ HYPOTHESIS NOT SUPPORTED: No clear advantage for biologically")
            print("   anchored encodings over arbitrary mappings.")
        
        print(f"\n🔍 DETAILED FEATURE COMPARISON")
        print(f"{'Feature':<20} {'Bio r':<10} {'Arb r':<10} {'Bio Better':<12} {'Difference':<12}")
        print("-" * 70)
        
        for feature, comp in comparison['bio_vs_arb_better'].items():
            better = "✓" if comp['bio_better'] else "✗"
            print(f"{feature:<20} {comp['bio_r']:>9.4f} {comp['arb_r']:>9.4f} {better:>11} {comp['difference']:>11.4f}")
...
def main():
    ...
    print(f"   Efficiency range: [{np.min(efficiencies):.3f}, {np.max(efficiencies):.3f}]")
    print()
    ...
    print(f"📁 Results saved and visualization generated.")
...
def create_results_visualization(results):
    ...
    ax1.axhline(y=0.5, color='red', linestyle='--', alpha=0.7, label='Significance threshold')
    ...
    for i, feature in enumerate(features):
        ax2.annotate(feature.replace('spectral_', '').replace('_', ' '), 
    ...
    significant_counts = [comparison['summary']['bio_significant_features'],
                         comparison['summary']['arb_significant_features']]
    
    bars = ax3.bar(categories, significant_counts, color=['#2E8B57', '#CD5C5C'], alpha=0.8)
    ...
    if 'bootstrap' in results['biological_encoder']:
        ...
        if features_with_bootstrap:
            ...
            bio_ci_lower = [bio_bootstrap[f]['ci_lower'] for f in features_with_bootstrap]
            bio_ci_upper = [bio_bootstrap[f]['ci_upper'] for f in features_with_bootstrap]
            
            x_pos_bootstrap = np.arange(len(features_with_bootstrap))
            ax4.errorbar(x_pos_bootstrap, bio_means, 
                        yerr=[np.array(bio_means) - np.array(bio_ci_lower),
                              np.array(bio_ci_upper) - np.array(bio_means)],
                        fmt='o', capsize=5, capthick=2, color='#2E8B57', alpha=0.8)
            ...
            ax4.text(0.5, 0.5, 'Bootstrap data\nnot available', 
                    ha='center', va='center', transform=ax4.transAxes)
            ax4.set_title('Bootstrap Analysis')
    
    plt.tight_layout()
    plt.savefig('/home/runner/work/wave-crispr-signal/wave-crispr-signal/experimental_validation_results.png', 
