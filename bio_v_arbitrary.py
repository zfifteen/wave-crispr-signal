...
class BiologicalEncoder:
    ...
    def __init__(self):
        ...
        pol_normalized = {k: 20 * (v - pol_min) / (pol_max - pol_min) - 10 
        ...
        stack_normalized = {k: 10 * (v - stack_min) / (stack_max - stack_min) - 5
    ...
    def encode_sequence(self, sequence: str) -> np.ndarray:
        ...
        for i, base in enumerate(sequence):
            if base in self.bio_weights:
                ...
                encoded_base = self.bio_weights[base] * context_weight * np.exp(1j * phase)
                encoded.append(encoded_base)
            else:
                encoded.append(0 + 0j)
        return np.array(encoded)
...
class EmpiricalValidator:
    ...
    def analyze_sequence_set(self, sequences: List[str], encoder, 
                           encoder_name: str) -> Dict[str, Any]:
        ...
        for seq in sequences:
            ...
            if len(seq) > 10:
                ...
                # Calculate Z score
                z_score = self.zeta_shift.compute_z_score(seq, spectrum_orig, spectrum_mut)
    ...
    def run_comparative_analysis(self, n_sequences: int = 100, 
                               n_arbitrary_trials: int = 10) -> Dict[str, Any]:
        ...
        bio_results = self.analyze_sequence_set(test_sequences, self.bio_encoder, "Biological_v1")
        
        print("Analyzing biological encoding (Method 2: Pairing-based)...")
        bio_results2 = self.analyze_sequence_set(test_sequences, self.bio_encoder2, "Biological_v2")
        ...
        for trial in range(n_arbitrary_trials):
            ...
            arb_result = self.analyze_sequence_set(test_sequences, arbitrary_encoder, 
                                                 f"Arbitrary_Trial_{trial}")
...
def plot_results(results: Dict[str, Any]) -> None:
    ...
    ax1.hist(bio_z, bins=20, alpha=0.6, label='Bio Method 1 (Multi-prop)', color='blue', density=True)
    ax1.hist(bio2_z, bins=20, alpha=0.6, label='Bio Method 2 (Pairing)', color='green', density=True)
    ax1.hist(arb_z_means, bins=10, alpha=0.7, label='Arbitrary Encodings', color='red', density=True)
    ...
    values = [results['summary']['bio1_mean_z'], 
              results['summary']['bio2_mean_z'],
              results['summary']['arbitrary_mean_z']]
    errors = [results['summary']['bio1_std_z'], 
    ...
    # Plot 4: Effect sizes
    effect_sizes = [results['statistical_test']['bio1_vs_arbitrary']['cohens_d'],
                   results['statistical_test']['bio2_vs_arbitrary']['cohens_d']]
    p_values = [results['statistical_test']['bio1_vs_arbitrary']['p_value'],
...
def main():
    ...
    print(f"Biological Method 1 Mean Z-Score: {summary['bio1_mean_z']:.4f} ± {summary['bio1_std_z']:.4f}")
    print(f"Biological Method 2 Mean Z-Score: {summary['bio2_mean_z']:.4f} ± {summary['bio2_std_z']:.4f}")
    print(f"Arbitrary Encoding Mean Z-Score:  {summary['arbitrary_mean_z']:.4f} ± {summary['arbitrary_std_z']:.4f}")
    print()
    
    print(f"Statistical Test Results:")
    print(f"Bio Method 1 vs Arbitrary:")
    ...
    print(f"  Significant: {'YES' if stats_result1['significant'] else 'NO'} (α = 0.05)")
    print()
    print(f"Bio Method 2 vs Arbitrary:")
    ...
    print(f"  Significant: {'YES' if stats_result2['significant'] else 'NO'} (α = 0.05)")
    print()
    ...
    for i, (method, stats_res) in enumerate([("Method 1", stats_result1), ("Method 2", stats_result2)], 1):
        ...
        print(f"  {method}: Cohen's d = {stats_res['cohens_d']:.4f} ({effect_desc} effect size)")
    ...
    any_bio_better = (stats_result1['significant'] and bio1_better) or (stats_result2['significant'] and bio2_better)
    ...
    if any_bio_better:
        ...
        print("1. The biological properties tested may not be optimal for spectral encoding")
        print("2. The Z Framework may need refinement to better capture biological meaning")
