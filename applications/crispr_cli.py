...
def write_output(results: List[Dict], output_file: Optional[str] = None, format: str = 'json'):
    """
...
def format_csv(results: List[Dict]) -> str:
    ...
    headers = ['sequence_name', 'guide_sequence', 'position', 'pam_sequence', 
               'on_target_score', 'gc_content', 'nhej_prob', 'mmej_prob', 'hdr_eff']
...
def design_guides_command(args):
    ...
    for seq_name, sequence in sequences.items():
        print(f"Designing guides for {seq_name} ({len(sequence)} bp)...", file=sys.stderr)
...
Examples:
  ...
  python crispr_cli.py batch-score guides.txt -o scores.csv -f csv
    ...
    parser.add_argument('--length', type=int, default=20, help='Guide length (default: 20)')
    parser.add_argument('--output', '-o', help='Output file (default: stdout)')
    parser.add_argument('--format', '-f', choices=['json', 'csv', 'tsv'], default='json',
    ...
    design_parser.add_argument('--num-guides', '-n', type=int, default=5,
                              help='Number of guides to return (default: 5)')
    design_parser.add_argument('--predict-repair', action='store_true',
                              help='Predict repair outcomes')
    ...
    score_parser.add_argument('--off-target', help='Off-target sequence for risk analysis')
    score_parser.add_argument('--target-context', help='Target context for repair prediction')
    
    # Batch score command
    batch_parser = subparsers.add_parser('batch-score', help='Score multiple guides from file')
    batch_parser.add_argument('guides_file', help='File with guide sequences (one per line)')
