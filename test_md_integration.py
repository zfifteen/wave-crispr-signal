...
from molecular_dynamics_framework import MolecularDynamicsZFramework, compare_md_vs_base_framework
...
def quick_md_integration_test():
    ...
    try:
        ...
        print(f"✓ MD calculation successful")
        print(f"  MD Z mean: {format_mpmath_for_display(results['md_z_mean'])}")
        print(f"  Enhancement factor: {format_mpmath_for_display(results['md_enhancement_factor'])}")
    except Exception as e:
    ...
    try:
        ...
        if comparison['results']:
            ...
            print(f"✓ Comparative analysis successful")
    ...
    try:
        ...
        if 'error' not in falsification:
            print(f"✓ Falsification test successful")
