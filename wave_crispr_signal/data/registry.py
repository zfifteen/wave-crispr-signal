...
class DatasetRegistry:
    ...
    def register(self, metadata: DatasetMetadata):
        ...
        if metadata.species != "Homo sapiens":
            logger.warning(f"Dataset {metadata.name} is not Homo sapiens: {metadata.species}")
    ...
    def load(
        ...
        if name not in self.datasets:
            raise ValueError(f"Dataset '{name}' not registered. Available: {list(self.datasets.keys())}")
    ...
    def split_dataset(
        ...
        if strategy == 'split_by_gene':
            ...
            if 'screen_id' not in df.columns:
                raise ValueError("Dataset must have 'screen_id' column for split_by_screen")
        ...
        logger.info(f"Split dataset ({strategy}): train={len(train_df)}, val={len(val_df)}, test={len(test_df)}")
