...
class KappaConfig(BaseModel):
    ...
    precision_dps: int = Field(
        ...
        ge=15,
        le=100,
        description="Decimal precision for mpmath"
    )
    scale: Optional[float] = Field(
        default=None,
        description="Scaling factor (defaults to 1/φ for discrete mode)"
    )
...
def kappa(
    ...
    for idx, n_i in enumerate(n_arr.flat):
        ...
        n_val = mp.mpf(int(n_i) if isinstance(n_i, (np.integer, np.int_)) else float(n_i))
